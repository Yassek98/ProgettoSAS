...
\begin{enumerate}
    ...
    \item  Seleziona un membro del personale
    \item \textbf{OPZIONALMENTE} decide di modificare le informazioni della persona selezionata, \textbf{OPPURE} decide di eliminare la persona dall'elenco del personale, in caso non sia più disponibile.\\
    ...
    \textbf{OPPURE ANCORA} consulta lo storico degli eventi e le note sulle performance per valutarne l'operato.\\
\textit{Se decide di promuovere il membro selezionato prosegue col passo 4, altrimenti termina il caso d'uso.}\\
\textbf{OPPURE ANCORA}\footnote{Azione eseguibile solo dal proprietario, per gestire una richiesta di ferie} valuta la richiesta di ferie  incrociando il monte ferie residuo con gli eventi pianificati. \\
\textit{Se decide di accettare la richiesta di ferie prosegue col passo 5, se no termina il caso d'uso}
        \item Modifica lo stato del personale selezionato da "occasionale" a "permanente"\footnote{Azione eseguibile solo dal proprietario, consultando gli organizzatori interessati, per gestire la carriera di un collaboratore}.
        \item Modifica lo stato del personale selezionato da "Disponibile" a "In ferie" per il periodo di ferie richiesto.
\end{enumerate}
\vspace{2\baselineskip}
...
\noindent\textbf{\large\color{red} Eccezione 2a.1a - Permessi non sufficenti}
\begin{center}
\renewcommand{\arraystretch}{1.5}
\rowcolors{1}{}{}
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.45\textwidth}|>{\raggedright\arraybackslash}p{0.45\textwidth}|}
...
\textbf{\#} & \multicolumn{1}{|c|}{\textbf{Attore}} & \multicolumn{1}{|c|}{\textbf{Sistema}} \\
\hline
   2a.1a.1& Seleziona l'opzione per aggiungere una nuova persona.&   Mostra messaggio di errore: l'attore che sta cercando di eseguire questa operazione non ha permessi sufficenti (solo il proprietario può aggiungere personale).\\\hline
 & \textit{Termina il caso d’uso}&\\\hline
\end{tabular}
\end{center}
\vspace{1.5\baselineskip}
...
\noindent\textbf{\large\color{red} Eccezione 3b.2a - \textbf{Permessi non sufficenti} }
\begin{center}
\renewcommand{\arraystretch}{1.5}
\rowcolors{1}{}{}
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.45\textwidth}|>{\raggedright\arraybackslash}p{0.45\textwidth}|}
...
operazione non ha permessi sufficenti (solo
...
\noindent\textbf{\large\color{red} Eccezione 3c.3a - \textbf{Permessi non sufficenti} }
\begin{center}
\renewcommand{\arraystretch}{1.5}
\rowcolors{1}{}{}
\begin{tabular}{|c|>{\raggedright\arraybackslash}p{0.45\textwidth}|>{\raggedright\arraybackslash}p{0.45\textwidth}|}
...
\begin{figure}[H]
    ...
    \hspace*{-1cm} 
    \includegraphics[width=1\linewidth, keepaspectratio]{input_file_2.png}
...
\begin{figure}[H]
    ...
    \includegraphics[width=0.73\textwidth]{input_file_1.png}
...
\begin{enumerate}
    ...
    \textbf{Post-condizioni:}
    \begin{itemize}[leftmargin=*,noitemsep,topsep=-5pt]
        \item[-] È stata creata un'istanza \texttt{\textit{collab}} di \texttt{Collaboratore}
    ...
    \begin{itemize}[leftmargin=*,noitemsep,topsep=-5pt]
        \item[-] è in corso la visualizzazione del profilo di \texttt{\textit{collab}}
        \item[-] non esiste un'istanza \texttt{\textit{t}} di \texttt{Turno} tale che \texttt{\textit{collab}} è \textbf{disponibile per} \texttt{\textit{t}} e \\ \texttt{t.data > data\_corrente}
    \end{itemize}
    \textbf{Post-condizioni:}
    \begin{itemize}[leftmargin=*,noitemsep,topsep=-5pt]
    ...
    \begin{itemize}[leftmargin=*,noitemsep,topsep=-5pt]
        ...
        \begin{itemize}
            ...
            \item[-] \texttt{\textit{collab}.monteFerie = \textit{collab}.monteFerie@pre - (\textit{rich}.dataFine - \textit{rich}.dataInizio)}
        \end{itemize}
    \end{itemize}
\end{enumerate}
\newpage
